...
Źródłowe obrazy PNG są skalowane do rozmiaru odpowiedniego dla aktywnego
wyświetlacza i zapisywane jako pliki RAW RGB565.
...
scripts/prepare_spiffs_logos.py
        |
        v
data/www/*.raw (tylko aktywny wariant)
        |
        v
...
| `DSP_ST7789` | 320×240 | 90×60 |
| `DSP_ILI9486` | 480×320 | 120×90 |
| `DSP_ILI9488` | 480×320 | 120×90 |
| `DSP_ST7796` | 480×320 | 120×90 |

Domyślne uruchomienie skryptu generuje wariant aktywnego wyświetlacza.
...
Rozmiar można jednorazowo nadpisać:

```powershell
python scripts/convert_logos.py --size 90x60
```

Jeśli w `myoptions.h` zostaną jawnie zdefiniowane obie wartości:
...
mają one pierwszeństwo przed rozmiarem przypisanym do `DSP_MODEL`.

## Wybór stacji z playlisty

Konwerter nie przetwarza automatycznie wszystkich obrazów z `logos_src`.
Odczytuje pierwszą kolumnę pliku:
...
5. Sprawdź podsumowanie konwersji. Dodana stacja powinna zostać zaliczona
   do pozycji korzystających z indywidualnego PNG, a nie z fallbacku lub
   `logo_default.png`. Skrypt utworzy odpowiedni plik `.raw` w katalogu
   wariantu `logos_raw`.
...
Limit 22 znaków wynika z limitu długości ścieżki SPIFFS:
...
Fallbacki są zgodne z konfiguracją `STATION_LOGO_FALLBACKS` w
`myoptions.h`. Pozwalają użyć jednego logo dla grupy stacji, np.:
...
Błąd na jednym z tych etapów nie usuwa istniejącego zestawu RAW. Konwerter
nie czyści bezpośrednio `data/www`.
...
Przed konwersją skrypt wyświetla m.in.:

```text
Playlista: 139 stacji
wybrano PNG: 81
fallbacki: 59
logo domyślne: 0
pominięto PNG: 90
