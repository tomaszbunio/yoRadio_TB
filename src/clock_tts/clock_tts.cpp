...
void clock_tts_loop() {
  ...
  if (clock_tts_fading_down) {
    ...
    if (clock_tts_fade_volume <= 0) {
      ...
      player.connecttospeech(buf, clock_tts_language); // przełącz strumień przy głośności=0 (brak pyknięcia)
      delay(150); // poczekaj na wypełnienie bufora TTS
      player.setVolume(clock_tts_prev_volume); // podnieś głośność gdy TTS zaczyna grać
  ...
  if (tm_struct) {
    ...
    if (clock_ttsActive && (nowMillis - clock_lastTTSMillis > 5000)) {
      player.setVolume(0); // wycisz przed powrotem do radia (brak pyknięcia przy stopSong TTS)
      clock_tts_fade_volume = 0; // fade-up startuje od 0, nie ustawia vol=0 ponownie
