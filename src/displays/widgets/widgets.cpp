...
void ClockWidget::_begin() {
    #ifdef PSFBUFFER
    #ifdef FLIP_CLOCK
    // Extended fb: VPAD px above glyph top + VPAD px below baseline
    _fb->begin(&dsp, _clockleft,
               _config.top - _timeheight - FLIP_PANEL_VPAD,
               _clockwidth, _flipPanelH + 1,
...
void ClockWidget::_printClock(bool force) {
    ...
    if (force) {
        ...
        #ifdef FLIP_CLOCK
        {
          ...
          strncpy(_prevTimebuffer, _timebuffer, sizeof(_prevTimebuffer));
          _drawFlipColon();  // natychmiast odśwież dwukropek (parzysta sekunda = widoczny)
        }
        #else
    ...
    if (_fullclock || _superfont > 0) { // Wyświetla także sekundy, a nie tylko godziny i minuty.
    // *** Wyświetl sekundy ***
    gfx.setTextSize(0);
    gfx.setFont(Clock_GFXfontPtr_Sec);
    ...
    #ifdef CLOCKFONT_MONO
        // cień nieaktywnych segmentów – celowy efekt wizualny
        gfx.setTextColor(config.theme.clockbg, config.theme.background);
        gfx.print("88");
    #else
        // kasowanie przez fillRect – działa poprawnie dla wszystkich czcionek
        uint16_t secW = _textWidth("88", Clock_GFXfontPtr_Sec);
        int8_t secH = 0;
        if (Clock_GFXfontPtr_Sec) {
            GFXglyph *g = pgm_read_glyph_ptr(Clock_GFXfontPtr_Sec,
                          '8' - pgm_read_byte(&Clock_GFXfontPtr_Sec->first));
            secH = pgm_read_byte(&g->height);
        }
        gfx.fillRect(leftSec, topSec - secH, secW + 2, secH + 2, config.theme.background);
    #endif

    // rysowanie aktualnych sekund – wspólne dla obu trybów
    gfx.setTextColor(config.theme.seconds, config.theme.background);
    gfx.setCursor(leftSec, topSec);
    sprintf(_tmp, "%02d", network.timeinfo.tm_sec);
    gfx.print(_tmp); // Wyświetl sekundy
}
    gfx.setTextSize(Clock_GFXfontPtr == nullptr ? TIME_SIZE : 1);
    gfx.setFont(Clock_GFXfontPtr);
    #ifndef FLIP_CLOCK
...
/**
 * @brief Rysuje lub ukrywa dwukropek między karteczkami HH i MM.
 *
 * Dwukropek miga co sekundę: widoczny gdy tm_sec parzyste, ukryty gdy nieparzyste.
 * Rysowanie bezpośrednio na dsp (nie przez _fb) – FlipDigit nie używa framebuffera.
 *
 * Obszar dwukropka jest kasowany fillRect przed każdym rysowaniem, żeby
 * usunąć poprzedni stan (widoczny ↔ ukryty).
 */
void ClockWidget::_drawFlipColon() {
    const uint16_t pw  = _flipDigitW * 2 + FLIP_DIGIT_PAD * 2;
    const uint16_t gap = FLIP_DIGIT_GAP;
    uint16_t colonW = _charWidth(':', Clock_GFXfontPtr);
    if (colonW == 0) colonW = pw / 4;
    // Górna krawędź obszaru dwukropka = baseline - wysokość tekstu - margines VPAD
    int16_t panelTop = _config.top - (int16_t)_timeheight - (int16_t)FLIP_PANEL_VPAD;
    // Pozycja X dwukropka = za karteczką HH + odstęp
    int16_t xColon   = _clockleft + (int16_t)pw + (int16_t)gap;

    // Skasuj poprzedni stan dwukropka (pełna wysokość karteczki)
    dsp.fillRect(xColon, panelTop, colonW, _flipPanelH, config.theme.background);
    // Rysuj dwukropek tylko na parzystych sekundach
    if (network.timeinfo.tm_sec % 2 == 0) {
        dsp.setFont(Clock_GFXfontPtr);
        dsp.setTextSize(1);
        dsp.setTextColor(0xFFFF);          // biały – pasuje do kolorystyki karteczek
        dsp.setCursor(xColon, _config.top);
        dsp.print(":");
    }
}

/**
 * @brief Pętla animacji flip clock i migania dwukropka.
 *        Wywoływana co ~10 ms z Display::loop() przez ClockWidget::tick().
 *
 * Dwie niezależne operacje:
 *   1. Mignięcie dwukropka – sprawdzenie czy sekunda się zmieniła (_lastSec).
 *      Gdy tak: redraw dwukropka (natychmiastowy, bez animacji).
 *   2. Klatki animacji FlipDigit – delegowane do _flipHH.loop() i _flipMM.loop().
 *      Każdy FlipDigit sam zarządza swoim tempem (FLIP_SPEED_MS / FLIP_FRAMES).
 */
void ClockWidget::tick() {
    if (!_active) return;

    // Mignięcie dwukropka: odśwież gdy sekunda się zmieniła
