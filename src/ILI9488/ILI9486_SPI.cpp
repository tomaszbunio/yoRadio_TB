...
void ILI9486_SPI::_writeColor16(const uint16_t* data, uint32_t n)
{
  ...
  // ESP8266: 60B (64B SPI FIFO). ESP32: 3072B (1024px/chunk) – DMA limit ~4092B.
  // Mniejszy bufor = więcej wywołań transferBytes (mutex+DMA setup per call) → audio dropy.
  // 3072B: ~9 wywołań dla 8280px scroll (vs 423 przy 60B) – redukuje overhead ~47x.
#if defined(ESP32)
  static const uint16_t rgb888_buffer_size = 3072;
#else
  static const uint16_t rgb888_buffer_size = 60;
#endif
  static const uint32_t max_chunk = rgb888_buffer_size / 3; // rgb888
  static uint8_t rgb888_buffer[rgb888_buffer_size]; // static → BSS (nie stos); brak re-entrancy bo SPI bus jest zajęty
  while (n > 0)
  {
    uint32_t chunk = min(max_chunk, n);
    n -= chunk;
