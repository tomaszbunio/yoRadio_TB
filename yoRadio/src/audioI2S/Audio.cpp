...
const char* Audio::parsePlaylist_PLS() {
    ...
    int     pos = 0;
    char*   host = nullptr;
Serial.printf("Audio.cpp->parsePlaylist_PLS() lines: %d \n", lines); 

for(int i = 0; i < lines; i++) {
 Serial.printf("Audio.cpp->parsePlaylist_PLS() m_playlistContent[%d]: %d \n", i, m_playlistContent[i].index_of("http", 0)); 

} 

    for(int i = 0; i < lines; i++) {
        if(i == 0) {
            Serial.printf("Audio.cpp->parsePlaylist_PLS() m_playlistContent[0].strlen(): %d \n", m_playlistContent[0].strlen()); //módosítás
...
void Audio::setVolume(uint8_t vol) {
    //Serial.printf("vol bemenet: %d", vol);
...
//    if(v < 207) AUDIO_INFO("Do not use this ancient Adruino version V%d.%d.%d", ESP_ARDUINO_VERSION_MAJOR, ESP_ARDUINO_VERSION_MINOR, ESP_ARDUINO_VERSION_PATCH);
    vol = (vol * 254) / 100;  //Visszaalakítja a 0-100 értéket a dekodernek  0-254 -re. // Módosítva. "vol_step"
    if(vol > 254) vol = 254;  // m_vol 0 - 254 -es érték.
    //if(vol < 0) vol = 0;
    m_vol = vol;
   // Serial.printf("m_vol kimenet: %d", m_vol);
    computeLimit();
}
//****************************************************************************************
uint8_t Audio::getVolume() {
     uint8_t vol = m_vol / 2.54; // Módosítva, új sor. "vol_step"
     return vol;
    //return m_vol;  
}
//****************************************************************************************
...
//                if(log) /*AUDIO_LOG_DEBUG*/AUDIO_INFO("PES PacketLength: %d", PES_PacketLength);
 //                bool PTS_flag = false; //Módosítás kommentelés
 //               bool DTS_flag = false;  //Módosítás kommentelés
                int flag_byte1 = packet[posOfPacketStart + 6] & 0xFF;
                int flag_byte2 = packet[posOfPacketStart + 7] & 0xFF;  (void) flag_byte2; // unused yet
               // if(flag_byte1 & 0b10000000) PTS_flag = true; //Módosítás kommentelés
             //   if(flag_byte1 & 0b00000100) DTS_flag = true; //Módosítás kommentelés
